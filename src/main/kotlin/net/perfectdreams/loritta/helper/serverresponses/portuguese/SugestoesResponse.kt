...
class SugestoesResponse : RegExResponse() {
    init {
        patterns.add("mo|onde|qual|existe|tem( )?jeito|ajuda|quer(o|ia)|tem algum ?(jeito)".toPattern(Pattern.CASE_INSENSITIVE))
        patterns.add("eu( )?posso|de|fa(ço|z(er)|z|s)|".toPattern(Pattern.CASE_INSENSITIVE))
        patterns.add("fazer( uma)?|".toPattern(Pattern.CASE_INSENSITIVE))
